...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  qmd_status: () => ({ installed: false, version: null }),
  qmd_memory_query: () => ({ available: false, hits: [] }),
  qmd_update_index: () => ({ available: false, collectionCreated: false, indexed: false }),
  scaffold_memory_vault: (args: { path: string }) => ({
    path: args.path,
    createdFiles: ['AGENTS.md', 'CLAUDE.md', 'wiki/index.md', 'wiki/log.md', 'raw/inbox/.gitkeep'],
    skippedFiles: [],
    gitInitialized: true,
    registered: true,
  }),
